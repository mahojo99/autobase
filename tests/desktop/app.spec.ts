...
test('Electron isolated renderer, live readiness, demo orchestration, approval, and persistence', async () => {
  ...
  await page.getByRole('button', { name: 'Settings & engines' }).click();
  await expect(page.getByText('Claude Agent SDK 0.3.266', { exact: true })).toBeVisible();
  await page.getByRole('button', { name: 'Explore the offline demo' }).click();
  ...
  await page.getByRole('textbox', { name: 'Message Relay' }).fill('Show an approval');
  await page.getByRole('button', { name: 'Send message' }).click();
  ...
  expect(before.bots.some((b) => b.name === 'Researcher')).toBe(true);
  await app.close();
...
test('Desktop keyboard focus, IPC validation and encrypted credential round trip', async () => {
  ...
  await expect(page.getByRole('textbox', { name: 'Message Relay' })).toBeFocused();
